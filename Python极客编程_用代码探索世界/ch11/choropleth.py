...
df = pd.DataFrame(df, columns=['Target Geo Id2',
                               'Geographic area.1',
...
counties = [dict(county, Density=popl_dens_dict[cid]) for cid, county in counties.items() if county['state'] not in EXCLUDED]
