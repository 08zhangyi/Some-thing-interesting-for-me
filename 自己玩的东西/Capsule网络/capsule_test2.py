...
for v_channel_num_i in range(v_channel_num):
    ...
    for v_y_i in range(v_y):
        for v_x_i in range(v_x):
            ...
            u_predict.append(u_predict_temp)  # 暂时先都堆在一起，然后要进行reshape
# 临时办法，尽量不要用reshape
u_predict = tf.reshape(tf.concat(u_predict, axis=-1), (-1, size_x, size_y, u_channel_num, v_capsule_num, v_x, v_y, v_channel_num))
